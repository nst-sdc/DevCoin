...
// Implement the fetchUserContributions function directly
export const fetchUserContributions = async (username: string): Promise<UserContribution[]> => {
  ...
  try {
    ...
    const commitContributions = commits.map(commit => ({
      ...
      status: 'completed',
      points: Math.floor((commit.additions + commit.deletions) / 20) + 5 // 5 base points + 1 per 20 lines
    }));

    return [...prContributions, ...issueContributions, ...commitContributions].sort(
...
/**
 * Fetches all repositories from the GitHub organization
 */
